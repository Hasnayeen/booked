        ...
        return $schema
            ->components([
                ...
                TextEntry::make('schedules_count')
                    ...
                    ->formatStateUsing(fn ($record) => $record->schedules()->count()),

                TextEntry::make('active_schedules_count')
                    ->label('Active Schedules')
                    ->formatStateUsing(fn ($record) => $record->activeSchedules()->count()),
                ...
                TextEntry::make('stops')
                    ->label('Stops')
                    ->listWithLineBreaks()
                    ->formatStateUsing(fn ($record) => collect($record->stops ?? [])->pluck('stop')->toArray())
                    ->placeholder('No stops defined'),

                TextEntry::make('boarding_points')
                    ->label('Boarding Points')
                    ->listWithLineBreaks()
                    ->formatStateUsing(fn ($record) => collect($record->boarding_points ?? [])->pluck('point')->toArray())
                    ->placeholder('No boarding points defined'),

                TextEntry::make('drop_off_points')
                    ->label('Drop-off Points')
                    ->listWithLineBreaks()
                    ->formatStateUsing(fn ($record) => collect($record->drop_off_points ?? [])->pluck('point')->toArray())
